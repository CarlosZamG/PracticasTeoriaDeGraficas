...
opc = True
while opc == True:
    print("""Trabajo realizado por el equipo 'COP'.
    Busqueda de árboles por anchura y profundidad""")
    M = lee_matriz()
    ...
    1) Matriz de adyacencia de un árbol por busqueda en anchura.
    2) Matriz de adyacencia de un árbol por busqueda en profundidad.""")
    opcMA = valida_entradas(0,0,2)
    if opcMA == 1:
        AM = generar_árbol_anchura(M)
        print("Matriz de adyacencia del árbol generado por busqueda en anchura")
        dibuja_matriz(AM)
    else:
        AM = generar_árbol_profundidad(M)
        print("Matriz de adyacencia del árbol generado por busqueda en profundidad")
        dibuja_matriz(AM)
    print("""Seleccione una de las siguientes opciones:
    1) ejecutar de nuevo.
    2) Salir.""")
    opc = valida_entradas(0,0,3)
